...
def handle_request(request):
    ...
    if len(request_lines) > 0:
        ...
        if len(parts) > 1:
            ...
            return f"htdocs{route}"

    return "index.php"  # Default route to load index.php


def create_temp_php_POST_file(post_data, display_php_content):
...
def create_temp_php_GET_file(get_data, get_content):
    temp_php_file_path = os.path.join(
        os.path.dirname(__file__), "temp_get_file.php")
    ...
    with open(temp_php_file_path, 'w') as temp_php_file:
        ...
        temp_php_file.write(");\n")
        temp_php_file.write('foreach ($data as $key => $value) {\n')
        temp_php_file.write('   $_GET[$key] = $value;\n')
        temp_php_file.write('}\n')
        temp_php_file.write('?>\n')
        temp_php_file.write(get_content)
...
# Get content of the linked file in GET method


def get_display_php_GET_content(linked_file):
    print("linked file in func ", linked_file)
    display_php_path = os.path.join(
        os.path.dirname(__file__), f"htdocs/{linked_file}")
    with open(display_php_path, 'r') as display_php_file:
        display_get_php_content = display_php_file.read()

    return display_get_php_content

# Get content of php file that is linked to POST method
...
# This function is use to serve php file
...
while True:
    ...
    try:
        ...
        extention = route.split('.')[-1]

        print("Route = ", route)

        # Extract query parameters if present (For Get method)
        query_parameters = {}
        if "?" in route:
            route, query_string = route.split("?", 1)
            query_parameters = dict(item.split("=")
                                    for item in query_string.split("&"))
            print("Query Para = ", query_parameters)

        # Check if it's a POST request and get the POST data
        post_data = {}
        php_content = "Not Found"
        if "POST /" in request:
            print("Req = ", request)
           ...
           # Read the request in a loop until the end of the request data
            ...
            if (extention == 'html'):
                ...
                # Serve the file with the query parameters (GET data)
                print("Linked file = ", linked_file)
                display_get_php_content = get_display_php_GET_content(
                    linked_file)
                temp_php_file_path = create_temp_php_GET_file(
                    query_parameters, display_get_php_content)
                php_content = run_php_file(temp_php_file_path)
        else:
